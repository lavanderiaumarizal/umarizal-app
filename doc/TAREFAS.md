...
| B3.1 | **Criar endpoint `PATCH /api/usuarios/:id/perfis`** | Endpoint admin para gerenciar perfis de um usuário. Body: `{ perfis: string[] }`. Atualiza `perfisApp` no banco. [Ver detalhamento](./SPRINT1_B3_1_DETALHAMENTO.md) | ✅ |
...
| B19 | **Criar endpoint `PUT /api/orcamentos/:id/etapa`** | Atualiza etapa específica com status e responsavel. Alternativa mais simples aos endpoints individuais. **Dispensada — substituída pelos endpoints individuais (B6–B8)** | 🔴 |
| B20 | **Criar filtro de dados por perfil** | Implementar função `filtrarDadosPorPerfil(orcamento, perfis)` que remove campos financeiros (valorTotal, pix, parcelas, etc.) e dados sensíveis (CPF, e-mail) para perfis não-admin. Aplicar em todos os endpoints de listagem/detalhes. *Implementado transversalmente: selects `SELECT_PADRAO`/`SELECT_ADMIN` no `kanban.service.ts` (doc 6)* | ✅ |
...
| F14.1 | **Mapa da Rota (MapLibre + OpenStreetMap)** | Mapa na tela Rota do Dia (componente `MapaRota.tsx` com `@maplibre/maplibre-react-native`, tiles OpenFreeMap — **sem chave de API**). Pins com cores diferentes para coleta (🟢) e entrega (🔵). Linha conectando as paradas na ordem da rota. Ao tocar no pin, mostrar nome + endereço + botão navegar | ✅ |
...
| F19 | **Integração com Mapas (deep link)** | Botão "📍 Maps" que abre o navegador/app de mapas com endereço destino via deep link (`https://maps.google.com/?daddr=endereco`, com fallback). Mapa em tela usa **MapLibre + OpenStreetMap (OpenFreeMap)** — sem chave de API (substituiu a proposta inicial de Google Maps) | ✅ |
| F19.1 | **Desabilitação automática de paradas** | Após coleta ou entrega bem-sucedida, atualizar o estado local da lista para marcar a parada como concluída. Ao recarregar a rota, o backend já retorna `concluido: true` nos waypoints já finalizados (B23) | ✅ |
...
| Q6 | **Build APK Final** | `npx eas-cli build --platform android --profile preview --non-interactive --no-wait`. APK **v4** gerado, assinado e instalado (login homologado em produção) | ✅ |
